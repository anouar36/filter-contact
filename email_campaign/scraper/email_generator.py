        ...
        try:
            ...
            if self.api_key:
                if self.openai_base_url:
                    self.client = OpenAI(api_key=self.api_key, base_url=self.openai_base_url)
                    self.provider = "OpenAI-Compatible"
                    logger.info(
                        f"OpenAI-compatible client initialized (base_url: {self.openai_base_url}, model: {self.model})"
                    )
                else:
